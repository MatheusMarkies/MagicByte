...
UNITY_SETUP_INSTANCE_ID(input);
	...
	input.positionOS.y = 0.005 * heightMap.b * abs(sin((90 * input.baseUV.y * 2))) * abs(sin((90 * input.baseUV.x * 2)));
...
float3 UvRecap(float3 positionWS,float tile) {
    float3 uv = float3(0,0,0);

	uv.r = positionWS.x * tile;
	uv.g = positionWS.z * tile;
	uv.b = _Time.y * 0.5;
...
float4 LitPassFragment(Varyings input) : SV_TARGET{
	...
	_displace = tex2Dlod(_HeightMap, float4(UvRecap(water.position, 0.1), 0));
	_offset = 1 * float3(0, 1, 0) * tex2Dlod(_HeightMap, float4(UvRecap(water.position, 2), 0));
	water.color = lerp(UNITY_ACCESS_INSTANCED_PROP(UnityPerMaterial, _BaseColor), _WaterBlendColor, _displace);
	water.alpha = 1;
