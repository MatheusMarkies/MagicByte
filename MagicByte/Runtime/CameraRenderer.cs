...
public partial class CameraRenderer {
	...
	ComputeShaderStackBlit csBlit = new ComputeShaderStackBlit();
	...
	public void Render (ScriptableRenderContext context, Camera camera,bool useDynamicBatching, bool useGPUInstancing,ShadowSettings shadowSettings) {
		...
		if (PPC)
		{
			PPC.setPPS(new PostProcessingStack());
			postStack = PPC.getPPS();
			postStack.Setup(context, camera);

			PPC.OnRenderCamera();

			if (PPC.getMaterialList() != null)
			postStack.PostProcessingDrawing(PPC,frameBufferId);



		}

		ComputeShaderStack CCS = camera.GetComponent<ComputeShaderStack>();
		if (CCS)
		{
			CCS.OnRederCamera();
			csBlit.Setup(context, camera);
			foreach (ExecuteComputeShader exe in CCS.getComputeShaderList())
			{
				exe.setCulling(cullingResults);
				csBlit.RenderComputeShader(exe.OnRenderComputeShader(),exe.getAccumulation(), exe.getFilterMaterial());
				exe.currentSample ++;
			}
		}
