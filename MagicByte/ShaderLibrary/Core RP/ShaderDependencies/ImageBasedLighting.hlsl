#ifndef UNITY_MAGICBYTE_IMAGE_BASED_LIGHTING_HLSL_INCLUDED
#define UNITY_MAGICBYTE_IMAGE_BASED_LIGHTING_HLSL_INCLUDED
...
float PerceptualRoughnessToMipmapLevel(float perceptualRoughness, float NdotR)
{
    ...
    float n = (2.0 / max(float_EPS, m * m)) - 2.0;
                   ...
                   bool usePrecomputedSamples)
{
    ...
    for (uint i = 0; i < sampleCount; ++i)
    {
        ...
        cbsdfInt += F * G;
    #else
        // Use the approximation from "float Shading in Unfloat Engine 4": Weight ~ NdotL.
        lightInt += NdotL * val;
        cbsdfInt += NdotL;
    #endif
    }
...
*/
    ...
    for (uint i = 0; i < sampleCount; i++)
    {
        ...
        if (NdotL > 0.0)
        {
            ...
            if (pdf > 0.0)
            {
                ...
                // CBSDF  = F * D * G * NdotL / (4 * NdotL * NdotV) = F * D * G / (4 * NdotV).
                // Weight = CBSDF / PDF.
                // We use two approximations of Brian Karis from "float Shading in Unfloat Engine 4":
                // (F * G ~ NdotL) && (NdotV == 1).
    ...
    cbsdfInt = max(cbsdfInt, float_EPS);
