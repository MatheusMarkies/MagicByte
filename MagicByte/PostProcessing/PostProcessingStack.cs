...
namespace MagicByte
{
	public class PostProcessingStack
	{
		...
		public void postProcessingDrawing(List<Effect> effects, int sourceId,Camera camera,bool usePathTracing,RenderTexture RT)
		{
			if (usePathTracing)
			{
				bufferPT.BeginSample("Path tracing Render");
				bufferPT.Blit(RT, BuiltinRenderTextureType.CameraTarget);
